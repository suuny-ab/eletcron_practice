...
from ..services import AIService, SessionCleanupService, KnowledgeService
...
class ServiceFactory:
    ...
    def get_ai_service(self) -> AIService:
        ...
        if not self._ai_service:
            chat_service = self.get_chat_model_service()
            knowledge_repository = self.get_knowledge_repository()
            self._ai_service = AIService(chat_service, knowledge_repository)
        return self._ai_service


    def snapshot(self) -> tuple[ModelProvider | None, ChatModelService | None, AIService | None]:
        """保存当前状态快照，用于回滚"""
        return (self._model_provider, self._chat_model_service, self._ai_service)

    def restore(self, snapshot: tuple[ModelProvider | None, ChatModelService | None, AIService | None]) -> None:
        """恢复到指定快照"""
        self._model_provider, self._chat_model_service, self._ai_service = snapshot
